...
class AnswerTable:
    ...
    def __init__(self, dsets=None):
        self.all_ans = json.load(open("data/gqa/lxmert/all_ans.json"))
...
# def load_lxmert_qa(path, model, label2ans, device):
#     """
...
#     :return:
#     """
#     path = '/media/dataset_aioz/GQA/lxmert/snap/pretrained/model'
...
#         if key.startswith('bert.') and key.startswith('bert.pooler.') is False:
...
def load_lxmert_qa(path, model, classifier, label2ans, device):
    """
    Load model weights from LXMERT pre-training.
    The answers in the fine-tuned QA task (indicated by label2ans)
        would also be properly initialized with LXMERT pre-trained
    ...
    if model is not None:
        ...
        bert_state_dict = {}
        for key, value in loaded_state_dict.items():
            if key.startswith('bert.'):# and not key.startswith('bert.pooler.') and not key.startswith('bert.encoder.x_layer.'):
                bert_state_dict[key] = value

        # Load Bert Weights
        model.model.load_state_dict(bert_state_dict, strict=False)
    ...
    if classifier is not None:
        ...
        for key, value in loaded_state_dict.items():
            if key.startswith("answer_head."):
                answer_state_dict[key.replace('answer_head.logit_fc.', '')] = value
        # Do surgery on answer state dict
        classifier_state_dict = classifier.state_dict()
        ans_weight = answer_state_dict['3.weight']
        ans_bias = answer_state_dict['3.bias']
        import copy
        new_answer_weight = copy.deepcopy(classifier_state_dict['3.weight'])
        new_answer_bias = copy.deepcopy(classifier_state_dict['3.bias'])
        answer_table = AnswerTable()
        loaded = 0
        unload = 0
        if type(label2ans) is list:
        ...
        print("Loaded %d answers from LXRTQA pre-training and %d not" % (loaded, unload))
        print()
        answer_state_dict['3.weight'] = new_answer_weight
        answer_state_dict['3.bias'] = new_answer_bias

        # Load Answer Weights
        classifier.load_state_dict(answer_state_dict, strict=False)

...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        # Run Ban model
        # joint_feats = self.ban(visn_feats, lang_feats)
        lang_cross_feats = lang_feats
        visn_cross_feats = visn_feats
        # Run cross-modality layers
        for layer_module in self.x_layers:
            lang_cross_feats, visn_cross_feats = layer_module(lang_cross_feats, lang_attention_mask,
                                                  visn_cross_feats, visn_attention_mask)

        return lang_feats, visn_feats, lang_cross_feats
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # Run LXRT backbone
        lang_feats, visn_feats, cross_feats = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats=visual_feats,
            visn_attention_mask=extended_visual_attention_mask)
        pooled_output = self.pooler(cross_feats)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
        ...
        lang_feats, visn_feats, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
...
class BertFeatureExtraction(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config, args):
        ...
        self.embeddings = BertEmbeddings(config)
        self.layer = nn.ModuleList(
            [BertLayer(config) for _ in range(self.args.llayers)]
        )
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None):
        ...
        lang_feats = self.embeddings(input_ids, token_type_ids)

        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, extended_attention_mask)
        return lang_feats
